...
## How to use
```javascript
var weeksNow = weekGenerator().run(); // returns an array of the current month divided by 6 lines (array), which one containing another array with 7 days.

var weeksDecember2016 = weekGenerator("2016, 12"); //returns the same as above, but for december 2016. You also can use "2016, dec".

var weekGen = weekGenerator(); // in this way, you can access other options, such as:
